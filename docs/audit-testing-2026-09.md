...
| T13 | **high** | **`spec_drift.py` and `spec_tests.py` only ever judged the tip commit in CI.** Both walk `git log`, and the checks job used the default shallow clone, so the log was one commit. Any unit followed by a non-unit commit passed without being checked — U299's own guarantee has been weaker on the runner than on a laptop since it was written. Found because U375c failed *Spec coverage* and the docs commit on top of it passed with identical code. | **done (U376)** — `fetch-depth: 0` on the checks job; U375c claimed. |
...
| T13 the traceability checks read the whole history in CI, not a one-commit clone | |

...
def test_person_description_and_skill_references() -> None:
    ...
    with TestClient(app) as client, tempfile.TemporaryDirectory() as tmp:
        ...
        assert client.put("/knowledge/people/nora", json={
            "display_name": "Nora", "role": "family",
        ...
        body = client.get("/knowledge/people/nora").json()
        assert body["person"]["display_name"] == "Nora"
        ...
        skills_api.get_store().save(Skill(
            name="playlist-for-nora", description="how Nora likes music picked",
            person="nora", body="Skate punk first.",
        ))
        body = client.get("/knowledge/people/nora").json()
        assert [s["name"] for s in body["skills"]] == ["playlist-for-nora"]
...
def test_description_lands_in_judgment_context() -> None:
    ...
    person = Person(person_id="nora", display_name="Nora", role="family",
                    description="my partner; prefers short answers")

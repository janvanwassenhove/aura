...
def test_wrong_endpoint_refiles_for_tagging() -> None:
    ...
    with TestClient(app) as client:
        ...
        client.put("/knowledge/people/tycho", json={"display_name": "Tycho", "role": "family"})
        snap = gallery.record("tycho", _png(), 0.71, embedding=[0.5, 0.6])

        r = client.post(f"/knowledge/people/tycho/snapshots/{snap.snapshot_id}/wrong")
        assert r.status_code == 200
        body = r.json()
        ...
        assert client.get("/knowledge/people/tycho/snapshots").json()["snapshots"] == []
        assert len(log.list()) == 1

        assert client.post("/knowledge/people/tycho/snapshots/bogus/wrong").status_code == 404

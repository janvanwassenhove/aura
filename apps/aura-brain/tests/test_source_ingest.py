...
def test_refresh_all_respects_per_person_optout(fake_fetch_and_llm) -> None:
    ...
    with TestClient(app) as client:
        client.put("/knowledge/people/jan", json={"display_name": "Jan", "role": "owner"})
        client.post("/knowledge/people/jan/facts", json={"key": "source:blog", "value": "https://blog.example"})
        client.put("/knowledge/people/nora", json={"display_name": "Nora", "role": "family"})
        client.post("/knowledge/people/nora/facts", json={"key": "source:blog", "value": "https://nora.example"})
        client.post("/knowledge/people/nora/facts", json={"key": "source-refresh", "value": "off"})
        ...
        assert by_person["nora"].get("skipped") == "auto-refresh off"

        # Toggling back on (a newer fact wins) re-includes them.
        client.post("/knowledge/people/nora/facts", json={"key": "source-refresh", "value": "on"})
        r2 = client.post("/knowledge/refresh-sources").json()
        by_person2 = {x["person_id"]: x for x in r2["refreshed"]}
        assert by_person2["nora"]["added_count"] == 2

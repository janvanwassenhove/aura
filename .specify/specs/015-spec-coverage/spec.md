...
- **FR-TRACE-02**: **Traceability runs both ways.** `spec_drift.py` asks
...
- **FR-GATE-01**: **The gate runs locally from the file CI reads.**
  ...
  stopping at the first failure as the runner does. There is no local copy of
  the step list to drift; a step added to the gate runs here on the next
  invocation (U372, audit T5).

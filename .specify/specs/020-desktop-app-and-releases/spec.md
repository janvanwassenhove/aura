...
- **FR-CI-01**: The job that runs the repository's own checks installs the
  ...
  *dependencies* directly above it; U361 then added a script importing
  `shared_schemas`, and CI was red for six units while the Release workflow
  stayed green and kept publishing installers. A test reads the top-level
  imports of every script and names any module the environment cannot provide,
  so the next drift is one readable line instead of seven failing assertions
  with a `ModuleNotFoundError` buried in captured stdout (U367).
